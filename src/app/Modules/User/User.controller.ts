...
const addUserReview = async (req: Request, res: Response, next: NextFunction) => {
  
  try {
    
    const result = await UserServices.addUserReviewIntoDB(
      req.params.id,
      req.body,
    );
    res.status(200).json({
...
const getAllCarReviews = async (req: Request, res: Response, next: NextFunction)  => {
  try {
    ...
    const result = await UserServices.getAllCarReviewsFromDB();
    res.status(200).json({
      success: true,
...
const getSingleCarReviews = async (req: Request, res: Response, next: NextFunction)  => {
  ...
  try {
    ...
    const result = await UserServices.getSingleCarReviewsFromDB(
      req.params.id,
    );
    res.status(200).json({
      success: true,
